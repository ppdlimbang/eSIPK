function SettingsPage() {
  const { authUser, view, schools, filesList, newSchoolCode, setNewSchoolCode, newSchoolName, setNewSchoolName, loading, editingSchool, setEditingSchool, editSchoolCode, setEditSchoolCode, editSchoolName, setEditSchoolName, handleAddSchool, handleEditSchoolSave, handleDeleteSchool, handleResetSchools, handleFileUpload, handleDeleteFile, inputClass } = useAppContext();
  return (
<>
            {view === 'settings' && authUser?.type === 'admin' && (
              <div className="fade-in max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-10">
                  ...
                  <p className="text-sm font-medium text-slate-500 mb-6 pl-12">Tambah atau kemas kini rujukan sekolah (Kod dan Nama) ke dalam ruangan pilihan pengisian.</p>

                  <form onSubmit={handleAddSchool} className="flex flex-col sm:flex-row gap-4 p-5 bg-slate-50/80 rounded-2xl border border-slate-200 shadow-inner">
                    <div className="w-full sm:w-1/3"><label htmlFor="settingspage-field-3" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-2">Kod Sekolah</label><input id="settingspage-field-3" type="text" value={newSchoolCode} onChange={(e) => setNewSchoolCode(e.target.value)} placeholder="Cth: YBA1234" className={inputClass} /></div>
                    <div className="flex-1"><label htmlFor="settingspage-field-4" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-2">Nama Sekolah <span className="text-rose-500">*</span></label><input id="settingspage-field-4" required type="text" value={newSchoolName} onChange={(e) => setNewSchoolName(e.target.value)} placeholder="Cth: SK CONTOH" className={inputClass} /></div>
                    <div className="flex items-end"><button type="submit" className="w-full bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-sm font-bold hover:bg-indigo-700 transition-colors shadow-sm flex items-center justify-center gap-2 h-[50px]"><span className="text-yellow-400"><Icons.Plus /></span> Tambah</button></div>
                  </form>
