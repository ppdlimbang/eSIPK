function SettingsPage() {
  ...
  const newPpdManaged = isPpdManagedLocation({ code: newSchoolCode, name: newSchoolName });
  const editPpdManaged = isPpdManagedLocation({ code: editSchoolCode, name: editSchoolName });
  return (
<>
            {view === 'settings' && isSettingsAdmin(authUser) && (
              <div className="fade-in max-w-5xl mx-auto space-y-8">
                ...
                <div className="bg-white rounded-[2rem] border border-slate-100 shadow-[0_8px_30px_rgba(0,0,0,0.04)] p-6 sm:p-10">
                  ...
                  <p className="text-sm font-medium text-slate-500 mb-6 pl-12">Daftar atau kemas kini akaun sekolah. Untuk Y050 FLAT PENDIDIKAN, emel dan kata laluan tidak diperlukan kerana unit ini di bawah PPD Limbang.</p>
                  ...
                  <form onSubmit={handleAddSchool} className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-4 p-5 bg-slate-50/80 rounded-2xl border border-slate-200 shadow-inner">
                    ...
                    <div><label htmlFor="school-email" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-2">E-mel Sekolah {!newPpdManaged && <span className="text-rose-500">*</span>}</label><input id="school-email" required={!newPpdManaged} disabled={newPpdManaged} type="email" autoComplete="off" value={newPpdManaged ? '' : newSchoolEmail} onChange={(e) => setNewSchoolEmail(e.target.value)} placeholder={newPpdManaged ? "Tidak diperlukan" : "sekolah@moe.gov.my"} className={inputClass} /></div>
                    <div><label htmlFor="school-password" className="block text-[10px] font-bold text-slate-400 uppercase tracking-widest mb-1.5 ml-2">Kata Laluan {!newPpdManaged && <span className="text-rose-500">*</span>}</label><input id="school-password" required={!newPpdManaged} disabled={newPpdManaged} minLength="8" type="password" autoComplete="new-password" value={newPpdManaged ? '' : newSchoolPassword} onChange={(e) => setNewSchoolPassword(e.target.value)} placeholder={newPpdManaged ? "Tidak diperlukan" : "Minimum 8 aksara"} className={inputClass} /></div>
                    <div className="md:col-span-2 xl:col-span-4 flex justify-end"><button type="submit" disabled={loading} className="w-full sm:w-auto bg-indigo-600 text-white px-8 py-3.5 rounded-2xl text-sm font-bold hover:bg-indigo-700 disabled:opacity-60 transition-colors shadow-sm flex items-center justify-center gap-2 h-[50px]">{loading ? 'Mendaftar...' : <><span className="text-yellow-400"><Icons.Plus /></span> {newPpdManaged ? 'Daftar Unit PPD' : 'Daftar Sekolah & Akaun'}</>}</button></div>
                  </form>
                        ...
                        if (isEditing) {
                          return (
                            <div key={index} className="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 bg-indigo-50/50 rounded-xl mb-1 border border-indigo-200">
                              ...
                              <label className="text-sm font-semibold">E-mel Sekolah<input type="email" disabled={editPpdManaged} value={editPpdManaged ? '' : editSchoolEmail} onChange={e => setEditSchoolEmail(e.target.value)} placeholder={editPpdManaged ? "Tidak diperlukan" : "sekolah@moe.gov.my"} className={inputClass} /></label>
                              <label className="text-sm font-semibold">Kata Laluan Baharu<input type="password" disabled={editPpdManaged} autoComplete="new-password" minLength={8} value={editPpdManaged ? '' : editSchoolPassword} onChange={e => setEditSchoolPassword(e.target.value)} placeholder={editPpdManaged ? "Tidak diperlukan" : "Kosongkan untuk kekalkan kata laluan"} className={inputClass} /></label>
